...
type Message struct {
	...
	BigHeadURL   string                 `json:"bigHeadURL"`         // 用户大头像
	SmallHeadURL string                 `json:"smallHeadURL"`       // 用户小头像
...
func (m *Message) ParseMediaInfo(data string) error {
	...
	case MessageTypeVoice:
		if msg.Voice.VoiceLength != "" {
			if duration, err := strconv.Atoi(msg.Voice.VoiceLength); err == nil {
				m.Contents["duration"] = duration
			}
		}
	...
	case MessageTypeAnimation:
		...
		m.Contents["aeskey"] = msg.Emoji.AesKey
	...
	case MessageTypeShare:
		...
		case MessageSubTypeText, MessageSubTypeLink, MessageSubTypeLink2:
			...
			m.Contents["displayname"] = msg.App.SourceDisplayName
...
func (m *Message) PlainTextContent() string {
	...
	case MessageTypeShare:
		...
		case MessageSubTypeQuote:
			...
			// 如果被引用的是图片，在引用行直接显示 [图片]
			var referContent string
			if refer.Type == MessageTypeImage {
				// 构造类似 PlainText 但内容替换为 [图片] 的字符串
				referBuf := strings.Builder{}
				rSender := refer.Sender
				if refer.IsSelf {
					rSender = "我"
				}
				if refer.SenderName != "" {
					referBuf.WriteString(refer.SenderName)
					referBuf.WriteString("(")
					referBuf.WriteString(rSender)
					referBuf.WriteString(") ")
				} else {
					referBuf.WriteString(rSender)
					referBuf.WriteString(" ")
				}
				referBuf.WriteString(refer.Time.Format("01-02 15:04:05"))
				referBuf.WriteString("\n[图片]\n")
				referContent = referBuf.String()
			} else {
